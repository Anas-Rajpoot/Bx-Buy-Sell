...
import { ChatLabelType, MessageType } from '@prisma/client';
...
export class ChatService {
  ...
  async getAllChats() {
    try {
      ...
      type ChatWithValidRelations = (typeof chats)[0] & {
        user: NonNullable<(typeof chats)[0]['user']>;
        seller: NonNullable<(typeof chats)[0]['seller']>;
      };
      ...
      const chats = await Promise.all(
        chatsRaw.map(async (chat) => {
          try {
            ...
            // Fetch chat label if exists (get first one since there can be multiple labels per chat)
